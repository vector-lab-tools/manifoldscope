/**
 * Next.js proxy to the Manifoldscope FastAPI backend.
 *
 * All requests to /api/backend/* are forwarded to http://localhost:8000/*.
 * Avoids CORS; matches the Vectorscope / Theoryscope pattern.
 */

import { NextRequest, NextResponse } from "next/server";

const BACKEND_URL =
  process.env.MANIFOLDSCOPE_BACKEND_URL ?? "http://localhost:8000";
...
async function forward(req: NextRequest, params: Promise<RouteParams>) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: "Manifoldscope backend unreachable.",
