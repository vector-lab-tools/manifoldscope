"""Training-Data Fingerprinting (Critique register, Phase 2a).

For each sample point, compute cosine similarity to each genre&apos;s marker
centroid and softmax into a distribution over genres. Reports the
per-concept genre fingerprint and the sample-level aggregate.
"""
...
PROBE_FILE = Path(__file__).parent.parent / "probes" / "genres.md"
...
def compute_training_data_fingerprint(
    ...
    dominant_genre = [names[int(i)] for i in dominant_idx]

    # Aggregate sample-level distribution: mean over rows.
    aggregate = dist.mean(axis=0).tolist()

    # Per-point tuples for the frontend table.
    per_point = []
    for i, concept in enumerate(items):
        per_point.append(
            {
                "item": concept,
                "dominant_genre": dominant_genre[i],
